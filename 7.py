# -*- coding: utf-8 -*-
from JhaiL.lineX import *
from JhaiL.service.ttypes import *
...
wait = {
    ...
    "comment":"Like like & like by ⊶ ɴᴅᴏᴋʙᴏᴛs ⊷",
    "message":"тᴇяıмᴀ кᴀsıн suᴅᴀн ᴀᴅᴅ sᴀʏᴀ \nвʏ.ᴛᴇᴀᴍ ⊶ ɴᴅᴏᴋʙᴏᴛs ⊷ \nline.me/ti/p/~team___ndok",
}
protect = {
    "pqr":[],
    "pinv":[],
...
def help():
    ...
    num = (num+1)
    helpMessage += "╠☙⇉ %i. " % num + key + "Addimage•nama•\n"
    num = (num+1)
    helpMessage += "╠☙⇉ %i. " % num + key + "Dellimage•nama•\n"
    num = (num+1)
    ...
    num = (num+1)
    helpMessage += "╠☙⇉ %i. " % num + key + "Batre •Cek skill bots"
    num = (num+1)
    helpMessage += "╚═─────◇─────═ \n"
    helpMessage +=  "   𝕁𝕙𝕒𝕚𝕝𝕓𝕠𝕥𝕤 𝕡𝕣𝕠𝕥𝕖𝕔𝕥─\n"
    return helpMessage
...
def helpbot():
    ...
    key = key.title()
    helpMessage2 =   "  ━────╮★╭────━\n"
    helpMessage2 += "      ─𝕁𝕙𝕒𝕚𝕝𝕓𝕠𝕥𝕤 𝕡𝕣𝕠𝕥𝕖𝕔𝕥─\n"
    helpMessage2 += "╔" + "─━━━━░★░━━━━━─\n"
    ...
    num = (num+1)
    helpMessage2 += "╠☙⇉ %i. " % num + key + "ndokceplok [kickall]\n"
    num = (num+1)
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in owner:
                               try:
                                   ...
                                   kb.sendMessage(msg.to,"si mantan sudah di buang ke laut bos")
                                   kb.sendMessage(msg.to, None, contentMetadata={"STKID":"51626509","STKPKGID":"11538","STKVER":"1"}, contentType=7)
                               except:
                                   pass
...
#===========KICKOUT============#       
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                             if msg.toType == 2:
                                 ...
                                 cl.sendMessage(msg.to,"「 Sorry r̸o̸o̸m̸ n̸y̸a̸ k̸a̸m̸i̸ s̸i̸t̸a̸ s̸e̸e̸ y̸o̸u̸ 」")
                                 targets = []
                                 for g in gs.members:
                        ...
                        elif text.lower() == 'ndokceplok':
                           if msg._from in owner or msg._from in admin or msg._from in staff:
                           	if msg.toType == 2:
                                  ginfo = cl.getGroup(msg.to)
                                  cl.sendMessage(msg.to, "Proses Cleanse....")
                                  cl.sendMessage(msg.to, "「 specialpakektelor 」\nmember : " +str(len(ginfo.members)) + " \nLOVE YOU...")
                                  G = cl.getGroup(msg.to)
                                  G.preventedJoinByTicket = False
                                  cl.updateGroup(G)
                                  Ticket = cl.reissueGroupTicket(msg.to)
                                  ...
                                  sw.acceptGroupInvitationByTicket(msg.to,Ticket)
                                  _name = text.lower().replace('ndokceplok','')
                                  gs = ki.getGroup(msg.to)
                                  gs = kk.getGroup(msg.to)
                                  gs = kc.getGroup(msg.to)
                                  gs = km.getGroup(msg.to)
